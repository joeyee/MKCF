...
import EOT_RMM_Feldmann_PerformAnalysis as eot_rmm
...
def load_frame(obj_name, trial_times, GtDict, roi_mask, start_id, end_id, ParamOptions):
    ...
    tracker_tbb_name  = resPath + obj_name + '_EOTRM_Tbbs.txt'
        ...
        while True:
            ...
            if frame.size != 0:
                ...
                if frame_id >= start_id and frame_id not in damaged_frames:
                    ...
                    cv2.putText(canvas_polar, 'frame ' + str(frame_id), org=(10, 50),
                    ...
                    cv2.putText(canvas_disp, obj_name+' frame ' + str(frame_id), org=(10, 50),
                                fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=1, color=(0, 255, 255),
                                thickness=2, lineType=cv2.LINE_AA)
                    ...
                    if frame_id == start_id :
                        ...
                        tracker = eot_rmm.rmm_ff()
                        ok = tracker.init(cx,cy, np.pi/2, obj_bbox[2], obj_bbox[3])
                        x_ = tracker.x0
                        X_ = tracker.X0
                        P_ = tracker.P0
                        alpha_ = tracker.alpha0
                    ...
                    if frame_id > start_id:
                        ...
                        if dist_min < 200 and len(blob_list)>0:
                            ...
                            N = len(idx)
                            measures = np.vstack((idx, idy))
                            meas_mean = np.mean(measures, 1)
                            meas_spread = (N - 1) * np.cov(measures)
                            #print 'meas_spread=', meas_spread
                            (x, X, P, alpha) = tracker.update(x_, X_, P_, alpha_, meas_mean, meas_spread, N)

                            _, len_RMM, ang_RMM = tracker.get_random_matrix_ellipse(X)
                            # print 'len_RMM = ', len_RMM
                            # print 'ang_RMM = ', ang_RMM
                            # print 'ang_RMM in Degree %f', ang_RMM * 180. / np.pi
                            #
                            rmm_par = np.hstack((np.dot(tracker.H, x), ang_RMM, np.real(len_RMM)))
                            # self.plot_extent(rmm_par, '-', 'g', 1)
                            # pylab.show(block=False)
                            (cx, cy) = np.real(rmm_par[0:2])
                            theta    = np.real(rmm_par[2])
                            (lx, ly) = np.real(rmm_par[3:5])

                            rotate_rect = ((cx,cy),(lx*2, ly*2), theta*180./np.pi)
                            ...
                            # print P
                            if (DETAIL_MODE==True):
                                print ('measures_number %d, rotated recc' % N, rotate_rect)
            ...
            #frame_counter +=1.
            #total_time_consum = total_time_consum + time.clock() - tstart
            #print ('frame %d cost %.2f seconds' % (frame_id, (time.clock() - tstart)))
